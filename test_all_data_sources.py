...
if result.get('sentiment') and result['sentiment'].get('success'):
    ...
    if sent.get('distribution'):
        ...
        print(f"      Positivo: {dist.get('positive', 0)} | Neutral: {dist.get('neutral', 0)} | Negativo: {dist.get('negative', 0)}")
...
if result.get('analysis') and result['analysis'].get('performance_score'):
    ...
    print(f"🎯 Rating: {result['analysis']['performance_score'].get('rating', 'N/A')}")
else:
    print("\n⚠️  Análisis: No disponible")

# Test 2: Marketplaces adicionales
