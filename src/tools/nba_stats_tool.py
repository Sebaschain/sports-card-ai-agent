...
class NBAStatsTool(BaseStatsTool):
    """Tool to fetch real NBA player statistics"""

    def __init__(self):
        self._name = "NBA Stats Tool"
        ...
        try:
            ...
            if len(career_df) > 0:
                ...
                return {
                    ...
                    "points_per_game": float(current_season["PTS"])
                    / float(current_season["GP"])
                    if current_season["GP"] > 0
                    else 0,
                    "assists_per_game": float(current_season["AST"])
                    / float(current_season["GP"])
                    if current_season["GP"] > 0
                    else 0,
                    "rebounds_per_game": float(current_season["REB"])
                    / float(current_season["GP"])
                    if current_season["GP"] > 0
                    else 0,
